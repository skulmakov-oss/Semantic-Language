...
impl QuadroReg {
	#[inline] pub fn new() -> Self { Self(0) }
	#[inline] pub fn from_raw(val: u64) -> Self { Self(val) }
	...
	pub fn try_set(&mut self, index: usize, state: u8) -> Result<(), &'static str> {
		if index >= 32 { return Err("Index out of bounds"); }
		if state > 0b11 { return Err("Invalid quadit state"); }
	...
	pub fn masks_all(self) -> QuadMasks {
		...
		QuadMasks { n: m_n, f: m_f, t: m_t, s: m_s }
	}
	...
	// Gates
	// --------------------------
	#[inline] pub fn merge(self, other: Self) -> Self { Self(self.0 | other.0) }
	#[inline] pub fn intersect(self, other: Self) -> Self { Self(self.0 & other.0) }
	#[inline] pub fn raw_delta(self, other: Self) -> Self { Self(self.0 ^ other.0) }
	...
	pub fn try_set_by_mask(self, mask_lsb: u64, state: u8) -> Result<Self, &'static str> {
		if (mask_lsb & MSB_MASK) != 0 { return Err("mask must be LSB-aligned"); }
		if state > 0b11 { return Err("invalid quadit state"); }
...
impl fmt::Debug for StateDelta {
	fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
		f.debug_struct("StateDelta")
			...
			.field("entered_false", &format_args!("{:#018x}", self.entered_false))
			.field("left_false",	&format_args!("{:#018x}", self.left_false))
			.field("entered_super", &format_args!("{:#018x}", self.entered_super))
...
impl<const N: usize> DeltaSoA<N> {
	...
	pub fn new() -> Self {
		Self {
			entered_true:  [0; N], left_true:  [0; N],
			entered_false: [0; N], left_false: [0; N],
			entered_super: [0; N], left_super: [0; N],
		}
	}
	...
	pub fn to_aos(&self) -> [StateDelta; N] {
		let mut out = [StateDelta {
			entered_true: 0, left_true: 0,
			entered_false: 0, left_false: 0,
			entered_super: 0, left_super: 0,
		}; N];
...
pub mod simd_opt {
	...
	pub fn bulk_merge(dest: &mut [QuadroReg], src: &[QuadroReg]) -> usize {
		...
		#[cfg(any(target_arch = "x86_64", target_arch = "x86"))]
		{
			if cfg!(target_feature = "avx2") { unsafe { return bulk_merge_avx2(dest, src, len); } }
			#[cfg(feature = "std")]
			if std::is_x86_feature_detected!("avx2") { unsafe { return bulk_merge_avx2(dest, src, len); } }
		}

		#[cfg(target_arch = "aarch64")]
		unsafe { return bulk_merge_neon(dest, src, len); }
	...
	pub fn bulk_intersect(dest: &mut [QuadroReg], src: &[QuadroReg]) -> usize {
		...
		#[cfg(any(target_arch = "x86_64", target_arch = "x86"))]
		{
			if cfg!(target_feature = "avx2") { unsafe { return bulk_intersect_avx2(dest, src, len); } }
			#[cfg(feature = "std")]
			if std::is_x86_feature_detected!("avx2") { unsafe { return bulk_intersect_avx2(dest, src, len); } }
		}

		#[cfg(target_arch = "aarch64")]
		unsafe { return bulk_intersect_neon(dest, src, len); }
	...
	pub fn bulk_inverse(dest: &mut [QuadroReg]) -> usize {
		...
		#[cfg(any(target_arch = "x86_64", target_arch = "x86"))]
		{
			if cfg!(target_feature = "avx2") { unsafe { return bulk_inverse_avx2(dest, len); } }
			#[cfg(feature = "std")]
			if std::is_x86_feature_detected!("avx2") { unsafe { return bulk_inverse_avx2(dest, len); } }
		}

		#[cfg(target_arch = "aarch64")]
		unsafe { return bulk_inverse_neon(dest, len); }
	...
	pub fn bulk_calc_delta(
		prev: &[QuadroReg], curr: &[QuadroReg],
		out_et: &mut [u64], out_lt: &mut [u64],
		out_ef: &mut [u64], out_lf: &mut [u64],
		out_es: &mut [u64], out_ls: &mut [u64],
	) -> usize {
		let len = prev.len();
		...
		#[cfg(any(target_arch = "x86_64", target_arch = "x86"))]
		{
			if cfg!(target_feature = "avx2") {
				unsafe { return bulk_calc_delta_avx2(prev, curr, out_et, out_lt, out_ef, out_lf, out_es, out_ls, len); }
			}
			#[cfg(feature = "std")]
		...
		#[cfg(target_arch = "aarch64")]
		unsafe { return bulk_calc_delta_neon(prev, curr, out_et, out_lt, out_ef, out_lf, out_es, out_ls, len); }
	...
	unsafe fn bulk_merge_avx2(dest: &mut [QuadroReg], src: &[QuadroReg], len: usize) -> usize {
		...
		while i + 4 <= len {
			...
			_mm256_storeu_si256(dest.as_mut_ptr().add(i) as *mut __m256i, _mm256_or_si256(a, b));
			i += 4;
		}
		i
	}
	...
	unsafe fn bulk_intersect_avx2(dest: &mut [QuadroReg], src: &[QuadroReg], len: usize) -> usize {
		...
		while i + 4 <= len {
			let a = _mm256_loadu_si256(dest.as_ptr().add(i) as *const __m256i);
			let b = _mm256_loadu_si256(src.as_ptr().add(i) as *const __m256i);
			_mm256_storeu_si256(dest.as_mut_ptr().add(i) as *mut __m256i, _mm256_and_si256(a, b));
			i += 4;
		}
		i
	}
	...
	unsafe fn bulk_inverse_avx2(dest: &mut [QuadroReg], len: usize) -> usize {
		...
		while i + 4 <= len {
			...
			_mm256_storeu_si256(dest.as_mut_ptr().add(i) as *mut __m256i, _mm256_or_si256(down, up));
			i += 4;
		}
		i
	}
	...
	unsafe fn bulk_calc_delta_avx2(
		...
		#[cfg(debug_assertions)]
		{
			for k in 0..i {
				...
				debug_assert_eq!(
					(out_et[k] | out_lt[k] | out_ef[k] | out_lf[k] | out_es[k] | out_ls[k]) & MSB_MASK,
					0
				);
			}
		}
...
impl<const NREG: usize> QuadroBank<NREG> {
	#[inline] pub fn new() -> Self { Self { regs: [QuadroReg::new(); NREG] } }

	#[inline]
	pub fn merge_inplace(&mut self, other: &Self) {
		#[cfg(all(feature = "simd", any(target_arch = "x86_64", target_arch = "x86", target_arch = "aarch64")))]
		...
		for i in start..NREG { self.regs[i] = self.regs[i].merge(other.regs[i]); }
	}
	...
	pub fn intersect_inplace(&mut self, other: &Self) {
		...
		for i in start..NREG { self.regs[i] = self.regs[i].intersect(other.regs[i]); }
	}
	...
	pub fn inverse_inplace(&mut self) {
		...
		for i in start..NREG { self.regs[i] = self.regs[i].inverse(); }
	}
	...
	pub fn calc_deltas_soa(prev: &Self, curr: &Self) -> DeltaSoA<NREG> {
		...
		let start = simd_opt::bulk_calc_delta(
			&prev.regs, &curr.regs,
			&mut soa.entered_true, &mut soa.left_true,
			&mut soa.entered_false, &mut soa.left_false,
			&mut soa.entered_super, &mut soa.left_super,
		);
...
pub mod bench {
	...
	fn finish(start: Instant, iters: u64) -> BenchResult {
		...
		BenchResult { iterations: iters, elapsed, ops_per_sec: (iters as f64) / secs }
	}
...
mod tests {
	...
	fn test_soa_simd_correctness_and_alignment() {
		...
		for i in 0..NREG {
			...
			let c_val = (i as u64).wrapping_mul(0xD1B5_4A32_D192_ED03).rotate_left(13);
