...
pub fn compile_program_to_ir_with_options(
        ...
        && cfg!(feature = "profile-logos")
    {
        return Err(FrontendError {
            pos: 0,
            message: "Logos input lowers to LogosIrLaw stream; EXObyte function IR requires RustLike frontend".to_string(),
        });
    }
...
pub fn compile_program_to_exobyte(input: &str) -> Result<Vec<u8>, FrontendError> {
    compile_program_to_exobyte_with_options(input, CompileProfile::RustLike, OptLevel::O0)
}

pub fn compile_program_to_exobyte_with_options(
    input: &str,
    profile: CompileProfile,
    opt: OptLevel,
) -> Result<Vec<u8>, FrontendError> {
    compile_program_to_exobyte_with_options_debug(input, profile, opt, false)
}

pub fn compile_program_to_exobyte_with_options_debug(
    input: &str,
    profile: CompileProfile,
    opt: OptLevel,
    debug_symbols: bool,
    ...
    emit_exobyte(ir.functions(), debug_symbols)
}

pub fn emit_ir_to_exobyte(funcs: &[IrFunction], debug_symbols: bool) -> Result<Vec<u8>, FrontendError> {
    emit_exobyte(funcs, debug_symbols)
}

fn emit_exobyte(funcs: &[IrFunction], debug_symbols: bool) -> Result<Vec<u8>, FrontendError> {
    let mut out = Vec::new();
    ...
    for f in funcs {
        ...
        let code = emit_exobyte_function(f, debug_symbols)?;
        write_u32_le(
            &mut out,
...
fn emit_exobyte_function(f: &IrFunction, debug_symbols: bool) -> Result<Vec<u8>, FrontendError> {
